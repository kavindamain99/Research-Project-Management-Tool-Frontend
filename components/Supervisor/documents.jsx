...
const SupervisorDocuments = () => {
    ...
    const dynamicImage = (document) => {
        if(document.docType === "marking") {
            return(
                <div className="col-md-4">
                    <img src={ markingImage } className="img-thumbnail" width="200px" alt="Document" />
                </div>
            );
        }
        if(document.docType === "document") {
            return(
                <div className="col-md-4">
                    <img src={ documentImage } className="img-thumbnail" width="200px" alt="Document" />
                </div>
            );
        }
    }

    const viewDocuments = documents.filter((data) => {
        if (searchDocuments === "" || data.name.toLowerCase().includes(searchDocuments.toLowerCase())) {
            return data;
        }
        }).map((document) => (
            <div className="card col-md-5" key={ document._id } style={{ "marginLeft" : "5%" }}>
                <div className="row">
                    ...
                    <div className="col-md-8">
                        <div className="card-body">
                            ...
                            <a href={ document.document } className="btn btn-danger mt-2 mb-2" download>
                                View Document
                            </a>
                            <p className="card-text"><small className="text-muted">Uploaded { createdDateTime(document) }</small></p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    const searchTop = () => !error && (
        <div>
            { MainNavBar() }
            <div id="input-group-prepend" className="row mt-3 mb-3">
                <span className="input-group-text" id="basic-addon1" style={{ width: "40px", marginLeft: "33.3%" }}>
                    <GoSearch />
                </span>
                <input className="form-control col-sm-3" type="search" style={{ width: "400px", }} placeholder="Search documents" name="searchForm" onChange={(event) => { setSearchDocuments(event.target.value); }}/>
            </div>
            <div className="row">
                { viewDocuments }
            </div>
