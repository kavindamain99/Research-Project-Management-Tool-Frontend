...
export const StudentDocuments = () => {
    ...
    const viewDocuments = () => !error && (
        <div>
            { MainNavBar() }
            <div className="row mt-4" style={{ "marginLeft" : "16%", "marginRight" : "16%" }}>
                <div className="col" style={{ "overflow-y" : "hidden" }}>
                    ...
                    { documents.map((document) => (
                        <div className="card shadow col-md-11 pl-4 mt-1 mb-1" key={ document.id } style={{ "marginLeft" : "10%" }}>
                            <div className="class-body">
                                <h5>{ document.groupId }</h5>
                                <p>{ document.type }</p>
                                <p className="card-text"><small className="text-muted">Submitted { createdDateTime(document) }</small></p>
                                <Link to={ `/supervisor/student/document/${ document._id }/${ document.groupId }` }>
                                    <button className="btn btn-block btn-danger">View Document</button>
                                </Link>
                            </div>
                        </div>
                    )) }
                ...
                <div className="col" style={{ "overflow-y" : "hidden" }}>
                    ...
                    { finalTheses.map((finalThesis) => (
                        <div className="card col-md-11 shadow p-2 mt-1 mb-1" key={ finalThesis.id }>
                            <h5>{ finalThesis.groupId }</h5>
                            <p>{ finalThesis.type }</p>
                            <p className="card-text"><small className="text-muted">Submitted { createdDateTime(finalThesis) }</small></p>
                            <Link to={ `/supervisor/student/document/${ finalThesis._id }/${ finalThesis.groupId }` }>
                                <button className="btn btn-block btn-danger">View Thesis</button>
                            </Link>
                        </div>
                    )) }
                </div>
