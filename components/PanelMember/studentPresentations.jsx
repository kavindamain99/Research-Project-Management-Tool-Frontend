...
const StudentPresentations = () => {
    ...
    const viewPresentations = presentations.filter((data) => {
        if (searchPresentations === "" || data.name.toLowerCase().includes(searchPresentations.toLowerCase())) {
            return data;
        }
        }).map((presentation) => (
        <div className="card col-md-4" key={ presentation._id }>
            <div className="card-body">
                ...
                <p className="card-text"><small className="text-muted">Uploaded { createdDateTime(presentation) }</small></p>
                <Link to={ `/panelmember/student/presentation/${ presentation._id }/${ presentation.groupId }` }>
    ...
    const searchUp = () =>  !error && (
        <div>
            ...
            <div id="input-group-prepend" className="row mt-3 mb-3">
                ...
                <input className="form-control col-sm-3" type="search" style={{ width: "400px", }} placeholder="Search presentations" name="searchForm" onChange={(event) => { setSearchPresentations(event.target.value); }}/>
            </div>
            <div className="row">
                { viewPresentations }
            </div>
